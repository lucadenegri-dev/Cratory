...
def test_anthropic_status_reflects_key(monkeypatch):
    monkeypatch.setattr("app.routers.providers.ai_tags.is_configured", lambda: True)
    body = client.get("/api/providers").json()
    assert next(p for p in body if p["key"] == "anthropic")["status"] == "configured"
    monkeypatch.setattr("app.routers.providers.ai_tags.is_configured", lambda: False)
    body = client.get("/api/providers").json()
    assert next(p for p in body if p["key"] == "anthropic")["status"] == "missing"


def test_acoustid_status_reflects_config(monkeypatch):
    monkeypatch.setattr("app.routers.providers.acoustid.acoustid_configured", lambda: True)
    monkeypatch.setattr("app.routers.providers.acoustid.fpcalc_available", lambda: True)
    body = client.get("/api/providers").json()
    assert next(p for p in body if p["key"] == "acoustid")["status"] == "configured"
    monkeypatch.setattr("app.routers.providers.acoustid.fpcalc_available", lambda: False)
    body = client.get("/api/providers").json()
