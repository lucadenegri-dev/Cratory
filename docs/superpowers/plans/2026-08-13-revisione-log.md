...
- [L3] `app/services/genre_norm.py` ≡ `app/organize/services/genre_norm.py` — **byte-identici** (37 righe, `diff` vuoto). Entrambi vivi. Fondere significa comunque toccare `organize/` — **RISOLTO, Task 8d**: riverificata l'identita' byte a byte (`diff` ancora vuoto, nessun drift dalla Fase 1). Fuso: `app/organize/services/genre_norm.py` ora re-esporta `normalize_genre` dal core (`from app.services.genre_norm import normalize_genre`); i due import esistenti in `organize/` (`genre_review.py`, `text_providers.py`) restano validi senza modifiche. Direzione: core canonico, `organize/` importa — e' il namespace assorbito dalla fusione. **Aggiornamento in review, stesso Task 8d**: lo shim stesso e' stato tolto di mezzo. I due importatori (`genre_review.py`, `text_providers.py`) ripuntati direttamente ad `app.services.genre_norm`; `app/organize/services/genre_norm.py` cancellato. Il test doppione `tests/organize/test_genre_norm.py` copriva un caso in piu' del gemello core (alias "idm" raggiunto con spazi attorno, `"  IDM "` -> `"IDM"`): portato nel parametrize di `tests/test_genre_chain.py` prima di cancellare il doppione. Risultato: una sola implementazione, nessun modulo ponte
- [L3] `app/services/native_picker.py` ≡ `app/organize/services/native_picker.py` — **byte-identici** (93 righe, `diff` vuoto). Entrambi vivi — **RISOLTO, Task 8d**: riverificata l'identita' byte a byte (ancora vuota). Fuso, ma NON con un `import` dei singoli simboli: i due file di test dedicati (`tests/test_native_picker.py` e la sua fotocopia `tests/organize/test_native_picker.py`) monkeypatchano `picker_available` e il lock privato `_lock` passando dal modulo del proprio percorso — un `from app.services.native_picker import pick_path` avrebbe lasciato i lookup interni di `pick_path` legati ai globals del modulo CORE, rendendo invisibile il monkeypatch fatto lato organize (provato: con quell'approccio `test_non_disponibile_solleva` lato organize falliva, "DID NOT RAISE"). Soluzione adottata: alias di `sys.modules['app.organize.services.native_picker']` verso il modulo core (`sys.modules[__name__] = _native_picker`), cosi' i due percorsi di import risolvono allo STESSO oggetto modulo — stesso `_lock`, stessi globals, stesso dict di attributi patchabile da entrambi i lati. 1914/1914 backend verdi dopo il merge, inclusi entrambi i file di test del picker e i due router (core e organize) che lo consumano. Effetto collaterale positivo, non richiesto ma corretto: i due router condividono ora la stessa mutua esclusione sul dialog nativo, che e' comunque unico a livello di sistema operativo (prima ogni modulo aveva il proprio `Lock()` indipendente). **Aggiornamento in review, stesso Task 8d**: l'alias di `sys.modules` e' stato tolto di mezzo a sua volta. Motivo: dopo la rimozione del router `organize/routers/picker.py` (Gruppo B, stesso task) lo shim non aveva piu' NESSUN importatore di produzione — solo `tests/organize/test_native_picker.py` lo importava, ed era una fotocopia quasi esatta del test sul modulo core. Un meccanismo la cui unica giustificazione era "far passare i test di un modulo che nessuno usa" e' un segnale che va tolto il modulo, non tenuto in vita con un trucco sugli import. Cancellati sia lo shim sia il test doppione; l'unico caso che il doppione copriva in piu' (`build_script` testato anche per `kind="file"`, non solo `"folder"`, sull'assert del timeout esplicito) portato nel test core superstite. `app/routers/files.py` resta l'unico chiamante di produzione, e importava gia' `app.services.native_picker` direttamente (mai lo shim)
...
- [L3] `GET /api/files/pick/availability` + `POST /api/files/pick` vs `GET /api/organize/picker/availability` + `POST /api/organize/picker/pick` — quattro endpoint, due router quasi verbatim (`app/routers/files.py:41-56` vs `app/organize/routers/picker.py` intero); differiscono solo per il prefisso, il nome dell'handler e da quale copia di `native_picker`/`http_errors` importano. Entrambe le coppie sono chiamate dal frontend — **Aggiornato, Task 8d**: la premessa "entrambe le coppie sono chiamate dal frontend" non vale piu' — riverificato con `/usr/bin/grep` che la coppia organize non ha piu' nessun chiamante (i client `pickerAvailability`/`pickPath` in `lib/organize/api.ts` erano gia' stati rimossi al Task 7). Rimossa la coppia organize per intero, incluso l'intero file `app/organize/routers/picker.py` (non aveva altro dentro) e la sua registrazione in `app/main.py`; la coppia core resta, ancora viva. Il servizio sopravvive fuso al core (vedi la voce `native_picker.py` qui sopra). Nota di seconda generazione: dopo questa rimozione lo shim `app/organize/services/native_picker.py` prodotto dal merge non ha piu' nessun importatore di produzione in `app/organize/` (solo il suo test dedicato `tests/organize/test_native_picker.py` lo importa) — segnalato in fondo al log sotto Task 8d, non rimosso in questa stessa passata
- [L3] `GET|PUT /api/settings/language` vs `GET|PUT /api/organize/settings/language` — **due store di lingua indipendenti per una app monoutente**, con default diversi: core usa `app_state` chiave `language` con default `"it"` (`app/services/app_state.py:47-54`), organize usa la colonna `Settings.language` con `DEFAULT_LANGUAGE` = EN (`app/organize/services/planning.py:42-54`). Evidenza aggiuntiva: i client organize `getLanguage`/`setLanguage` (`frontend/lib/organize/api.ts:514-519`) **non hanno nessun chiamante nel frontend** (0 hit) — la UI usa solo `/api/settings/language` (`frontend/lib/i18n/index.tsx:37,58`). Unificare e' una decisione di prodotto, non un merge meccanico — **Aggiornato, Task 8d**: rimossa solo la superficie HTTP organize (`GET|PUT /api/organize/settings/language`, i due handler `get_language_route`/`put_language_route` in `app/organize/routers/settings.py`; il resto del router, `GET|PUT /api/organize/settings` per i template naming/folder, resta intatto). Lo store NON toccato: colonna `Settings.language`, `planning.get_language`/`set_language`, default EN — restano esattamente come descritti in questa voce, e la loro copertura di test e' sopravvissuta spostandosi in `tests/organize/test_planning.py` (era mescolata con la copertura del router ormai rimosso in `tests/organize/test_language_setting.py`, cancellato). La superficie core `GET|PUT /api/settings/language` non toccata. L'unificazione dei due store resta esplicitamente fuori scope, decisione di prodotto. Nota di seconda generazione: lo schema `LanguageSetting` in `app/organize/schemas.py` non ha piu' nessun importatore dopo la rimozione dei due handler — segnalato in fondo al log sotto Task 8d, non rimosso
...
- [L3] `lib/api/format.ts:9` (`fmtDuration`) + `:17` (`fmtDate`) vs `lib/organize/api.ts:543` (`fmtDuration`) + `:550` (`fmtDate`) — **omonimi con comportamento diverso, entrambi vivi**. `fmtDuration`: la copia organize fa `Math.round(seconds)` prima di dividere, quella core no (sui float sputa secondi decimali). `fmtDate`: la copia organize e' consapevole della lingua (`getCurrentLanguage()`, locale `it-IT`/`en-GB`) e include ora e minuti; quella core **cabla `it-IT` a prescindere dalla lingua attiva** e mostra solo la data. Fondere richiede di scegliere il comportamento vincente per ogni call site — decisione di prodotto. **Nota a parte, e' un bug i18n vero:** `lib/api/format.ts:20` rende date in italiano anche con la UI in inglese (call site: `app/page.tsx:82`, `app/playlists/page.tsx:151`, `app/library/page.tsx:306`, `app/shazam/page.tsx:150`, ...). **RISOLTO, Task 8c**: decisione dell'utente di correggere subito il bug i18n (la duplicazione strutturale `fmtDuration`/`fmtDate` core vs organize resta segnalata qui sopra, non toccata). Sia `fmtDate` (`:17`) sia **`fmtDateShort` (`:25`, non nominata da questo finding ma affetta dallo stesso bug — vedi anche il finding trackLabel qui sotto)** ora derivano la locale da `getCurrentLanguage()` (`it` -> `it-IT`, `en` -> `en-GB`, stessa mappatura gia' in uso nella copia organize). Test rosso-poi-verde in `tests/format.test.ts` (per `fmtDateShort` l'assert e' sull'argomento locale passato a `toLocaleDateString`, non sulla stringa risultante: `it-IT` ed `en-GB` producono la stessa stringa dd/mm/yy per costruzione CLDR)
- [L3] `lib/api/format.ts:4-5` — `trackLabel()` ricade su `"Artista sconosciuto"` / `"Senza titolo"` **hard-coded in italiano, a prescindere dalla lingua attiva**. Stessa classe del bug `it-IT` di `fmtDate` qui sopra, stesso file, ma superficie piu' ampia: la funzione e' l'etichetta standard di una traccia in tutta la UI — 7 file, ~14 call site (`app/tracks/[id]/page.tsx:47`, `app/sets/[id]/page.tsx:390,442,464,496`, `app/transitions/page.tsx:110,126,168`, `app/wishlist/page.tsx:250`, `components/wishlist-row.tsx:99`, `app/playlists/import-manual/page.tsx:282,303`). Non e' codice morto: e' una segnalazione. ~~Il fix non e' meccanico — `format.ts` non importa il dizionario e non deve farlo alla leggera (`lib/i18n/runtime.ts:3` vieta l'import inverso da `lib/api`), quindi o si passa `t` come parametro a tutti i call site o si usa `getCurrentLanguage()` come fa gia' la copia organize di `fmtDate`~~ **Correzione (Task 8c): questa premessa era sbagliata.** Il divieto in `lib/i18n/runtime.ts:2-3` e' **unidirezionale** — vieta l'import **da** `lib/api` **dentro** `runtime.ts`, non il contrario — e la sua stessa docstring nomina esplicitamente `lib/api.ts` come consumatore legittimo dello stato lingua fuori-React. `lib/api/client.ts:1` importa gia' `translateApiError` da `@/lib/i18n/runtime` senza creare nessun ciclo. **RISOLTO, Task 8c**: decisione dell'utente di correggere. `trackLabel()` ora importa `getCurrentLanguage`/`DICTIONARIES` da `@/lib/i18n/runtime` e riusa le chiavi esistenti `tracks.unknownArtist`/`tracks.untitledHeading` (stesso valore IT dei letterali precedenti: nessuna regressione visiva in italiano). Nessun ciclo: `runtime.ts` importa solo `./en` e `./it`, che non importano nulla da `lib/api`. Test rosso-poi-verde in `tests/format.test.ts`
...
- [L3] Endpoint backend che restano senza chiamante frontend **se** il Task 7 esegue le rimozioni L1 qui sopra: `GET /api/organize/scan/status` (perdeva `scanJobStatus`), `GET /api/organize/fingerprint/status` (`fingerprintStatus`), `GET|PUT /api/organize/settings/language` (`getLanguage`/`setLanguage`), `GET /api/organize/picker/availability` + `POST /api/organize/picker/pick` (`pickerAvailability`/`pickPath`). Le ultime due famiglie sono **gia' segnalate in Fase 1** come duplicazione core <-> `organize/`; le prime due sono nuove. Nessuna azione qui: sono nel backend, e il piano vieta L1/L2 dentro `app/organize/`. Da valutare insieme, non uno alla volta — **Aggiornato, Task 8d**: valutati insieme come qui prescritto, con verifica indipendente per ciascuno (grep su `frontend/` e `backend/tests`, non solo sul frontend). Rimossi 3 su 4: `fingerprint/status`, `settings/language` (solo la coppia organize), `picker/*` (l'intera coppia organize). `GET /api/organize/scan/status` invece LASCIATO: a differenza degli altri tre ha chiamanti reali via HTTP in `backend/tests/organize/test_analyze_api.py` e `test_api.py`, che lo interrogano con `TestClient` per attendere il completamento dello scan job prima di proseguire con asserzioni su tutt'altro (l'endpoint analyze, il flusso end-to-end dello scan) — non e' copertura del solo endpoint sotto test, e' un chiamante genuino per uno scopo diverso. Dettagli per endpoint nelle voci dedicate qui sopra
...
- [L3] `lib/api/format.ts:4-5` — `trackLabel()` ricade su `"Artista sconosciuto"` / `"Senza titolo"` **hard-coded in italiano, a prescindere dalla lingua attiva**. Stessa classe del bug `it-IT` di `fmtDate` qui sopra, stesso file, ma superficie piu' ampia: la funzione e' l'etichetta standard di una traccia in tutta la UI — 7 file, ~14 call site (`app/tracks/[id]/page.tsx:47`, `app/sets/[id]/page.tsx:390,442,464,496`, `app/transitions/page.tsx:110,126,168`, `app/wishlist/page.tsx:250`, `components/wishlist-row.tsx:99`, `app/playlists/import-manual/page.tsx:282,303`). Non e' codice morto: e' una segnalazione. ~~Il fix non e' meccanico — `format.ts` non importa il dizionario e non deve farlo alla leggera (`lib/i18n/runtime.ts:3` vieta l'import inverso da `lib/api`), quindi o si passa `t` come parametro a tutti i call site o si usa `getCurrentLanguage()` come fa gia' la copia organize di `fmtDate`~~ **Correzione (Task 8c): questa premessa era sbagliata.** Il divieto in `lib/i18n/runtime.ts:2-3` e' **unidirezionale** — vieta l'import **da** `lib/api` **dentro** `runtime.ts`, non il contrario — e la sua stessa docstring nomina esplicitamente `lib/api.ts` come consumatore legittimo dello stato lingua fuori-React. `lib/api/client.ts:1` importa gia' `translateApiError` da `@/lib/i18n/runtime` senza creare nessun ciclo. **RISOLTO, Task 8c**: decisione dell'utente di correggere. `trackLabel()` ora importa `getCurrentLanguage`/`DICTIONARIES` da `@/lib/i18n/runtime` e riusa le chiavi esistenti `tracks.unknownArtist`/`tracks.untitledHeading` (stesso valore IT dei letterali precedenti: nessuna regressione visiva in italiano). Nessun ciclo: `runtime.ts` importa solo `./en` e `./it`, che non importano nulla da `lib/api`. Test rosso-poi-verde in `tests/format.test.ts`
- [L3] `app/playlists/import-spotify/liked/page.tsx` (160 righe) vs `app/playlists/import-soundcloud/likes/page.tsx` (159 righe) — **quasi fotocopie**: normalizzando i nomi di piattaforma il `diff` si riduce a ~15 righe (il campo di ricerca e' `artist` vs `uploader`, la chiave `spotify_id` vs `track_id`, due note di marginalia in piu' su SoundCloud, e una `clearSelection` estratta da una parte e inline dall'altra). Tutta l'impalcatura — stato, preview, filtro, selezione, import, tabella — e' identica. **Non L2** perche' nessuna delle due pagine ha copertura: non sono in `ROUTES` di `e2e/smoke.spec.ts` e non hanno test unitari; estrarre un componente condiviso e parametrizzarlo su due modelli dati diversi e' un refactor a occhi chiusi
- [L3 — trovato in review del Task 8c, mai nominato prima] Due parenti della stessa famiglia di bug (`Date` renderizzata senza seguire la lingua attiva), con esiti opposti verificati con `node -e` prima di scrivere questa voce. `components/clock.tsx:9` — `new Date().toLocaleTimeString("it-IT", { hour12: false })` e' hard-coded su `it-IT`, ma con `hour12: false` e sole ore/minuti/secondi numerici **ogni locale rende lo stesso identico output** (verificato `it-IT`/`en-GB`/`en-US`/`en`): hard-code **inerte**, nessuna azione. `app/playlists/[id]/page.tsx:588` invece — `new Date(ev.created_at).toLocaleString()`, **nessuna locale ne' opzioni** — segue il locale di sistema del browser invece della lingua attiva dell'app (es. `"3/5/2026, 3:23:45 PM"` su sistema en-US, formato e lingua indipendenti da `getCurrentLanguage()`): e' un bug **vivo**, stessa classe di `fmtDate`/`trackLabel` risolti al Task 8c ma mai segnalato prima e **fuori dallo scope autorizzato di quel task** (limitato ai due bug in `lib/api/format.ts`). Segnalato, non toccato

Ricadute sul backend delle rimozioni L1 del Task 7:

- [L3] Endpoint backend che restano senza chiamante frontend **se** il Task 7 esegue le rimozioni L1 della Fase 2: `GET /api/organize/scan/status` (perdeva `scanJobStatus`), `GET /api/organize/fingerprint/status` (`fingerprintStatus`), `GET|PUT /api/organize/settings/language` (`getLanguage`/`setLanguage`), `GET /api/organize/picker/availability` + `POST /api/organize/picker/pick` (`pickerAvailability`/`pickPath`). Le ultime due famiglie sono **gia' segnalate in Fase 1** come duplicazione core <-> `organize/`; le prime due sono nuove. Nessuna azione qui: sono nel backend, e il piano vieta L1/L2 dentro `app/organize/`. Da valutare insieme, non uno alla volta — **Aggiornato, Task 8d**: vedi l'esito nella voce gemella in Fase 1 qui sopra — rimossi `fingerprint/status`, `settings/language` (coppia organize) e `picker/*` (coppia organize); `scan/status` lasciato per chiamanti reali in `backend/tests`
...
- `lib/api/format.ts` — due bug i18n distinti, non codice morto: `fmtDate` (e,
  ...
  finding originale) cablava `it-IT` a prescindere dalla lingua attiva (7 call site
  `fmtDate` + 2 `fmtDateShort`, ricontati con grep al Task 8c e poi corretti in review
  — un ottavo hit iniziale, `app/organize/history/page.tsx:84`, e' l'omonimo `fmtDate`
  importato da `@/lib/organize/api`, non la funzione qui corretta; il finding originale
  ...
  `runtime.ts`, non il contrario); `lib/api/client.ts` importa gia' da li' senza
  ciclo. **RISOLTO, Task 8c**: decisione dell'utente di correggere entrambi i bug.
  `fmtDate`/`fmtDateShort` derivano la locale da `getCurrentLanguage()` (`it-IT`/
  `en-GB`); `trackLabel()` legge `DICTIONARIES[getCurrentLanguage()].tracks.{unknownArtist,untitledHeading}`.
  Dettagli e test rosso-poi-verde nei due finding L3 sopra (sezione *Segnalazioni*,
  Frontend Task 6) e in `tests/format.test.ts`.
...
## Task 8d — Duplicazione strutturale core <-> organize, parte meccanica

Eseguita la parte meccanica autorizzata dall'utente della duplicazione core <->
`organize/` catalogata in Fase 1 (le voci L3 sopra sono state annotate sul posto con
l'esito). Riassunto:
...
/api/organize/picker/availability` + `POST /api/organize/picker/pick`), verificati
uno per uno con `/usr/bin/grep` su `frontend/` E `backend/tests` come richiesto dal
brief — non solo sul frontend, che aveva gia' perso i client al Task 7. `GET
/api/organize/scan/status` lasciato: unico dei quattro con chiamanti reali via HTTP
in `backend/tests/organize/test_analyze_api.py` e `test_api.py`, che lo usano come
infrastruttura di polling per attendere il completamento dello scan job prima di
proseguire con asserzioni su tutt'altro (non copertura del solo endpoint). Ogni
servizio sottostante sopravvive intatto: `planning.get_language`/`set_language`
(store lingua organize, default EN, non toccato — l'unificazione con
`/api/settings/language` resta fuori scope), `native_picker` (fuso al core nel
Gruppo A), `POST /api/organize/fingerprint` (router sopravvissuto, solo lo `/status`
rimosso), `GET|PUT /api/organize/settings` per i template (router sopravvissuto).

**Codice morto di seconda generazione emerso dall'interazione fra i due gruppi:**

- [L3 — **RISOLTO IN REVIEW, stesso Task 8d**] `app/organize/services/native_picker.py`
  — lo shim di alias creato dal merge del Gruppo A aveva come unico importatore di
  produzione `app/organize/routers/picker.py`, cancellato subito dopo nel Gruppo B.
  Segnalato qui come orfano (0 importatori di produzione, solo un test doppione),
  ma inizialmente non rimosso per rispettare la regola "non cascare nello stesso
  respiro". La review del coordinatore ha rimesso in discussione quella prudenza:
  se l'unica giustificazione rimasta per un meccanismo e' "far passare i test di un
  modulo che nessuno usa", il meccanismo va tolto insieme al modulo. Verificato che
  fosse davvero cosi' (0 importatori in `app/`, un solo test doppione,
  `tests/organize/test_native_picker.py`, fotocopia quasi esatta del gemello core) e
  rimossi entrambi — vedi l'aggiornamento nella voce L3 dedicata qui sopra per i
  dettagli e il caso di copertura portato prima di cancellare
- [L3] `app/organize/schemas.py:190` — `LanguageSetting` (Pydantic, `{language:
  Literal["it","en"]}`) non ha piu' nessun importatore dopo la rimozione dei due
  handler `get_language_route`/`put_language_route` in
  `app/organize/routers/settings.py` (Gruppo B). E' distinta dall'omonima
  `LanguageSetting` di `app/routers/settings.py` (core, superficie viva): stessa
  duplicazione di nome gia' notata altrove in questa revisione per store paralleli.
  **Ancora non rimossa**: a differenza del punto sopra non e' un modulo-ponte creato
  da questa stessa revisione, e' uno schema Pydantic preesistente rimasto senza uso
  per una rimozione di endpoint (non di merge) — resta segnalata per una prossima
  passata mirata, non e' stata riaperta dalla review

**Verifica (dopo la review)**: backend 1888 passed, 4 deselected. Percorso completo
dell'aritmetica: 1914 (baseline pre-Task 8d) - 9 (Gruppo B: rimozione dei 3 endpoint
orfani, dettagliato sopra) = 1905 - 17 (`tests/organize/test_native_picker.py`,
cancellato per intero con lo shim) - 1 (`tests/organize/test_genre_norm.py`,
cancellato per intero con lo shim) + 1 (nuovo caso nel parametrize di
`tests/test_genre_chain.py`, portato dal doppione genre_norm prima di cancellarlo;
il caso portato da native_picker non aggiunge una `def test_` nuova, allarga un
