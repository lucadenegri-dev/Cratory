...
solo segnalazione, non rimosso in questa passata:**

- [L3] `app/organize/services/native_picker.py` — lo shim di alias creato dal
  merge del Gruppo A aveva come unico importatore di produzione
  `app/organize/routers/picker.py`, cancellato subito dopo nel Gruppo B. Da questa
  passata in poi il file non ha piu' nessun importatore in `app/organize/`: solo
  `tests/organize/test_native_picker.py` (copertura duplicata, fotocopia di
  `tests/test_native_picker.py` sul modulo core) lo importa ancora. Non rimosso
  nella stessa passata per la regola del brief ("il servizio sottostante deve
  sopravvivere a meno che non diventi a sua volta orfano — e se lo diventa,
  segnalarlo come nuovo finding invece di rimuoverlo nello stesso respiro"): il file
  e i suoi 17 test restano, in attesa di una decisione esplicita in una prossima
  passata (rimuovere lo shim e riorientare `tests/organize/test_native_picker.py` a
  importare dal core, oppure lasciarlo come garanzia extra che l'alias di
  `sys.modules` continui a funzionare nel tempo)
- [L3] `app/organize/schemas.py:190` — `LanguageSetting` (Pydantic, `{language:
  ...
  Non rimossa nella stessa passata, stessa motivazione del punto sopra

**Verifica**: backend 1905 passed, 4 deselected (era 1914 passed, 4 deselected: -9
test, tutti coverage esclusiva degli endpoint rimossi nel Gruppo B — 6 in
`tests/organize/test_picker_router.py`, cancellato per intero col router; 3 in
`tests/organize/test_language_setting.py`, che ne aveva 4: i 3 che testavano il
router via HTTP sono stati cancellati, il quarto — sulla sola funzione di servizio
`planning.get_language`/`set_language`, che sopravvive — spostato in
`tests/organize/test_planning.py`). Frontend invariato: lint 0 errori/4 warning
pre-esistenti, build 27/27 route, test:unit 186/186 (le modifiche di questo task
sono solo backend; verificato comunque il frontend per intero dopo ciascuno dei due
commit, come richiesto dal brief). Report completo:
`.superpowers/sdd/task-8d-report.md`.
