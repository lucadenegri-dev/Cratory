...
| **F3b** Via `scan_root` | rimozione di `scan_root`/`root_id`/`root_targets` e della pagina Sources (83 riferimenti) | ✅ **completata 2026-08-12** — eseguita in due tempi, F3a (modello) e F3b (`scan_root` fuori dal dominio): i target del piano derivano da Settings, nessuna regressione su Piano e Apply, 1844 test verdi. La rimozione **di schema** — la colonna `audio_file.root_id` e la tabella `scan_root` — era rimandata a dopo F4; **decisa in F6: non si fa**. F4 non ha richiesto alcun rebuild di `audio_file`, quindi il vincolo che impedisce il `DROP COLUMN` è immutato. Si riapre solo se una modifica futura richiede comunque quel rebuild, e in quel caso escono insieme a costo marginale zero |
