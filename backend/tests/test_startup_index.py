"""Avvio: se LIBRARY_ROOT è configurata, la scansione parte da sola."""
...
def test_avvio_lancia_indicizzazione_se_dovuto(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(scan_job, "get_state", lambda db, k: None)  # mai indicizzato → dovuto
    monkeypatch.setattr(scan_job, "start_job", lambda: called.append(True) or {})
...
def test_avvio_salta_indicizzazione_se_recente(monkeypatch, tmp_path):
    ...
    called = []
    monkeypatch.setattr(settings, "library_root", str(tmp_path))
    monkeypatch.setattr(scan_job, "get_state",
