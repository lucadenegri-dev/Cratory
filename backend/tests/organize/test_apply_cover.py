...
from app.organize.models import AudioFile, Plan, PlanOp, ScanRoot, UndoJournal
...
def test_apply_cover_embeds_and_sets_flag(db, copy_fixture, tmp_path):
    ...
    with patch("app.organize.services.apply.cover_art.fetch_image", return_value=_JPG):
...
def test_apply_cover_skips_on_download_failure(db, copy_fixture, tmp_path):
    from app.organize.integrations.cover_art import CoverArtError
    plan, f, fpath = _seed_cover_plan(db, copy_fixture, tmp_path)
    with patch("app.organize.services.apply.cover_art.fetch_image", side_effect=CoverArtError("dead")):
        res = apply_svc.apply_plan(db, plan)
...
def test_apply_cover_skips_when_has_cover_already_true(db, copy_fixture, tmp_path):
    ...
    f.has_cover = True
    db.commit()
    with patch("app.organize.services.apply.cover_art.fetch_image", side_effect=AssertionError(
...
def test_apply_cover_skips_on_write_failure_without_halting(db, copy_fixture, tmp_path):
    ...
    plan, f, fpath = _seed_cover_plan(db, copy_fixture, tmp_path)
    with patch("app.organize.services.apply.cover_art.fetch_image", return_value=_JPG), \
         patch("app.organize.services.apply.tagio.write_cover",
