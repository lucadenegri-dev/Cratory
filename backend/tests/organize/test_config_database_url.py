"""Fix review finale (F1): DJORG_DATABASE_URL relativo va riancorato a
...
from app.organize.core.config import BACKEND_DIR, Settings


def test_relative_database_url_is_anchored_to_backend_dir(monkeypatch):
    monkeypatch.delenv("DJORG_DATABASE_URL", raising=False)
...
def test_relative_cache_dirs_are_anchored_to_backend_dir(monkeypatch):
    monkeypatch.delenv("DJORG_COVER_CACHE_DIR", raising=False)
    monkeypatch.delenv("DJORG_THUMB_CACHE_DIR", raising=False)
