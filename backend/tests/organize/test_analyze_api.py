    ...
    with TestClient(app) as client:
        ...
        while time.time() < deadline:
            if client.get("/api/organize/scan/status").json()["status"] in ("done", "error"):
                break
            time.sleep(0.02)
        resp = client.post("/api/organize/analyze")
...
def test_decisions_survive_reanalyze(db):
    ...
    with TestClient(app) as client:
        ...
        r = client.post("/api/organize/analyze")
        ...
        kr = client.post(f"/api/organize/duplicates/{grp_id}/keeper", json={"file_id": non_keeper})
        ...
        issues = client.get("/api/organize/issues", params={"type": "missing_metadata"}).json()
        assert len(issues) >= 1
        ...
        dr = client.post(f"/api/organize/issues/{issue_id}/status", json={"status": "dismissed"})
        assert dr.status_code == 200

        # --- Step 4: seconda analisi (re-analyze) ---
        r2 = client.post("/api/organize/analyze")
        assert r2.status_code == 200

        # --- Step 5: verifica che le decisioni siano sopravvissute ---
        # Keeper override preservato
        groups2 = client.get("/api/organize/duplicates").json()
        ...
        issues2 = client.get("/api/organize/issues", params={"type": "missing_metadata"}).json()
