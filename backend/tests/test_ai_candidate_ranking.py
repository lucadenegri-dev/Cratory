...
Senza start/end_bpm dichiarati l'ancora del taglio a MAX_CANDIDATES non deve
...
from app.services.ai_agent import MAX_CANDIDATES, _rank_candidates
...
def test_no_bpm_constraint_does_not_pick_only_the_slowest():
    ...
    slowest_ids = {t.id for t in sorted(cands, key=lambda t: t.bpm)[:MAX_CANDIDATES]}
    assert {t.id for t in ranked} != slowest_ids
    # ...quindi circa metà della selezione sta sopra la mediana del pool.
    assert sum(1 for t in ranked if t.bpm > median) >= MAX_CANDIDATES // 3
...
def test_start_bpm_only_keeps_anchoring_to_start():
    ...
    cands = [_mk(i, 80.0 + i) for i in range(101)]
    ranked = _rank_candidates(cands, _req(start_bpm=100))
    expected = sorted(cands, key=lambda t: (abs(t.bpm - 100), t.id))[:MAX_CANDIDATES]
...
def test_no_bpm_constraint_ranking_is_deterministic():
    ...
    cands = [_mk(i, 80.0 + i) for i in range(101)]
    a = _rank_candidates(list(cands), _req())
    b = _rank_candidates(list(reversed(cands)), _req())
