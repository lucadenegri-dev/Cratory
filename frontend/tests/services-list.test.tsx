...
describe("ServicesList", () => {
  it("rende una riga per ogni servizio", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Soulseek" })).toBeTruthy();
    expect(screen.getByRole("heading", { name: "Anthropic" })).toBeTruthy();
  });

  it("Discogs resta disponibile senza token e i dettagli partono chiusi", async () => {
    render(<ServicesList services={SEVEN} />);
    expect(within(screen.getByRole("region", { name: "Discogs" })).getByText("Disponibile")).toBeTruthy();
    expect(screen.queryByRole("textbox")).toBeNull();
    expect(screen.queryByText(/non configurata|not configured/i)).toBeNull();
  });

  it("slskd senza URL: la riga dice «Da configurare», non «Disconnesso»", async () => {
    slskdStatusFn.mockResolvedValue({
      configured: false, reachable: false, is_connected: false, is_logged_in: false,
      is_connecting: false, is_transitioning: false, state: null, username: null,
      unauthorized: false, web_url: null,
    });
    daemonStatusFn.mockResolvedValue({ reachable: false, owned: null, pid: null, installed: true, configured: false, username: null });
    const services = SEVEN.map((s) => (s.key === "slskd" ? { ...s, configured: false } : s));
    render(<ServicesList services={services} />);
    const row = within(screen.getByRole("region", { name: "Soulseek" }));
    expect(await row.findByText("Da configurare")).toBeTruthy();
    await waitFor(() => expect(slskdStatusFn).toHaveBeenCalled());
    expect(row.queryByText("Disconnesso")).toBeNull();
  });
  ...
  it("soundcloud: salva l'username col valore ripulito", async () => {
    ...
    render(<ServicesList services={SEVEN} />);
    fireEvent.click(screen.getByRole("button", { name: "Gestisci SoundCloud" }));
  ...
  it("acoustid configurato: 'Identifica ora' chiama runFingerprint", async () => {
    ...
    render(<ServicesList services={SEVEN} />);
    fireEvent.click(screen.getByRole("button", { name: "Gestisci AcoustID" }));
