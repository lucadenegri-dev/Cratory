...
describe("pickSurprise", () => {
  ...
  it("pool vuoto -> null", () => {
    expect(pickSurprise({ genres: [], labels: [] }, [])).toBeNull();
  });
  ...
  it("pick uniforme deterministico con RNG fisso: primo elemento + prima depth", () => {
    ...
    const pick = pickSurprise(POOL, [], seqRng([0, 0]));
  ...
  it("il seedType segue il gruppo di provenienza (label)", () => {
    ...
    const pick = pickSurprise(POOL, [], seqRng([0.99, 0.5]));
    expect(pick).toEqual({ seedType: "label", value: "Trax Records", depth: 0.5 });
  });

  it("depth uniforme sui tre valori", () => {
    expect(pickSurprise(POOL, [], seqRng([0, 0.99]))?.depth).toBe(1.0);
    expect(pickSurprise(POOL, [], seqRng([0, 0.5]))?.depth).toBe(0.5);
  });

  it("anti-ripetizione: esclude il seme corrente (case-insensitive)", () => {
    // exclude = ["acid house"]: resta [Techno, Trax Records]. rng=0 -> Techno.
